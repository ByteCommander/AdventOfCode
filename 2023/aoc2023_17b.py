...
# while always going between 4 and 10 steps in a straight line at a time.
# Idea: write an A*, but instead of 1 edge per field, treat any sequence of 4-10 straight steps between curves
# as a single edge in the network graph. So from one node, you can end up on the nodes 4-10 left to it,
# or 4-10 right to it. You can't go further straight because that would have been covered already by the previous node.
# This means every step has up to 14 potential next steps, and any field can be visited twice, approaching it
...
def next_nodes(grid: list[list[int]], y: int, x: int, v: bool) -> list[(int, int, int)]:
    ...
    ds = [*range(-10, -3)] + [*range(4, 11)]  # distances to walk: [-10..-4] plus [4..10] inclusive
