...
async def main():
    ...
    min_x, max_x = min(x for x, y in squares), max(x for x, y in squares)
    min_y, max_y = min(y for x, y in squares), max(y for x, y in squares)
    print(min_x, max_x, min_y, max_y)
    print(*sorted((k for k, v in squares.items() if v), key=lambda xy:xy[1]), sep="\n")
    print("The robot paints the registration identifier below:\n")
    for y in range(min_y, max_y + 1):
        print("".join(" #"[squares.get((x, y), BLACK)] for x in range(min_x, max_x + 1)))
