...
def main(file: TextIO):
    ...
    print(f"The total calibration result supporting concatenation operators is {calibration}.")


def recurse(target: int, result: int, args: list[int]) -> bool:
    arg, *args = args
    for op in [concat, mul, add]:
        res = op(result, arg)
        # print(target, "=?", res, "->", result, op.__name__, arg, "...", args)
        if not args:
            if res == target:
                return True
